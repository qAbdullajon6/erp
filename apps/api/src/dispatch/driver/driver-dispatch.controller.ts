...
import { ReportIntermediateStopFailureDto } from "./dto/report-intermediate-stop-failure.dto";
...
export class DriverDispatchController {
  ...
  reportFailure(
    ...
    return this.driverDispatches.reportFailure(user.organizationId, user.userId, id, dto, user);
  }

  @Post(":id/stops/:stopId/fail")
  reportIntermediateStopFailure(
    @Param("id") id: string,
    @Param("stopId") stopId: string,
    @Body() dto: ReportIntermediateStopFailureDto,
    @CurrentUser() user: CurrentUserPayload,
  ) {
    return this.driverDispatches.reportIntermediateStopFailure(
      user.organizationId,
      user.userId,
      id,
      stopId,
