...
/// Members and invitations answer one question — who can get into this
/// workspace — so they share a section. They were briefly split apart, which
/// put an "Invite member" button in two places and made "did that invitation
/// land?" a navigation problem. They stay visually separate here, because an
/// invited person is not yet a member and has a different set of actions.
export function MembersSection() {
  ...
  const handleReactivate = async (membershipId: string) => {
    try {
      ...
      toast.error(describeError(err, 'Failed to reactivate member'));
    }
  };
  ...
  return (
    <SettingsSection
      ...
      description={
        isLoading || isError
          ? 'Everyone who can sign in to this workspace, and the invitations still waiting to be accepted.'
          : `${activeCount} active ${activeCount === 1 ? 'member' : 'members'} of ${members?.length ?? 0} total. A removed member keeps their history and can be reactivated.`
      }
      ...
      <div className="grid gap-8">
        <div className="overflow-hidden rounded-lg border border-brand/10">
          {isLoading ? (
            ...
            <ErrorState
              message={describeError(error, 'Failed to load members')}
              onRetry={() => refetch()}
            />
          ) : !members?.length ? (
        ...
        <section aria-labelledby="pending-invitations-heading" className="grid gap-3">
          <div className="grid gap-1">
            ...
            <p className="text-sm text-muted-foreground">
              People invited to join who have not accepted yet. An invitation expires on its own if it
              is never used.
            </p>
          </div>
          <div className="overflow-hidden rounded-lg border border-brand/10">
