...
>(
  (
    ...
    return (
      <div
        ref={ref}
        // lg, not md — see MOBILE_BREAKPOINT in hooks/use-mobile.tsx. These
        // queries decide what is visible; the hook decides what renders, and the
        // two have to agree or there is a width where both or neither appear.
  ...
  ({ className, ...props }, ref) => {
    return (
      <main
        ref={ref}
        className={cn(
          // min-w-0 is load-bearing, not tidying. As a flex child this element's
          // implicit min-width is `auto`, i.e. the intrinsic width of its
          // content — so a wide table or a full topbar could push it past the
          // viewport and scroll the whole document sideways instead of scrolling
          // inside its own container. It only showed up between 768px and about
          // 1100px, where the rail is already taking 16rem but the viewport is
          // still narrow, and the responsive sweep tested 1440 and 390 only.
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        // Increases the hit area of the button on touch widths (below lg, where
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        // Increases the hit area of the button on touch widths.
        "after:absolute after:-inset-2 after:lg:hidden",
